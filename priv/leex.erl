...
-export([gen/2,gen/3,format_error/1]).
...
gen(In, Out, Options) ->
    InFile = mod_name(In) ++ ".xrl",
    OutFile = mod_name(Out) ++ ".erl",
    ModName = mod_name(Out),
...
mod_name(M) when atom(M) -> atom_to_list(M);
mod_name(M) when list(M) -> M.
...
out_file(OutFile, Mod, DFA, DF, Actions, Code) ->
    ...
    case file:path_open(["../priv", [code:lib_dir(),"/tools/include"]],
			"leex.hrl", read) of
	{ok,Ifile,Iname} ->
